# Python Study Logbook 🐍

**Autore:** Matteo  
**Periodo di studio:** Dicembre 2025 – [in corso]  
**Tags:** #python #programmazione #logbook #apprendimento  

**Legenda:**  
- 📖 = Studio su libro
- 🔍 = Approfondimento concetto
- 🎥 = Lezioni video breve
- 🧑🏼‍🏫 = Lezioni video lunghe
- 📝 = Esercizi elementari
- 📐 = Esercizio complicato (progetto)
- 💭 = Esercizio Q&A (teoria)
- 🔰 = Studio per badges e certificazioni
- 📦 = GitHub

**Abbreviazioni:**  
- Python Crash Course (PCC)
- Pensare in Python (TP)
- Corey Shafer (PPB, Python Programming Beginner)
- Harvard Computer Science Python (CS50P)
- Python Essentials 1 (PE1)


---
## 2025-12-27
📖 PCC - Environment (Capitolo 1)  
🎥 PPB - Installation and Setup  
📝 `HelloWorld.py`  

## 2025-12-28
📖 PCC - Variables and Simple Data (Capitolo 2)   
🎥 PPB - Strings  
🎥 PPB - Integer & Floats  
🔍 [Stringhe](../03-CONCEPTS/Stringhe.md)  
📝 Stringhe (13 script)  
📦 Primo commit

## 2025-12-29
📖 PCC - Introducing Lists (Capitolo 3)  
🎥 PPB - List, Tuples and Sets  
📝 Liste (11 esercizi)  
🔍 Mutabilità con [esercizio](../02-EXERCISES/02-Lists/2_06_mutabilita.py)  
🔍 [Liste](../03-CONCEPTS/Liste.md)  
🔍 [Manipolare Liste](../03-CONCEPTS/ManipolareListe.md)

## 2025-12-30
📖 PCC - Working with lists, `for` Loop (Capitolo 4)  
📝 `for` (5 script)  
🎥 PPB - Dictionaries  

## 2025-12-31
📝 `for` (8 script)  
📐 `SingleRecordParser.py` [[Code Review]](../06-PROJECTS/01-BioParser/02_report_string_data_cleaning.md)  
🧑🏼‍🏫 CS50P - Functions & Variables [[Lezione 0]](../04-COURSES/CS50P/Lecture00.md)    
📦 Aggiunto [README.md](../README.md)  
📦 Aggiunto [BOOK_PROGRESS.md](BOOK_PROGRESS.md)
📦 Aggiunto [EXERCISES_PROGRESS.md](EXERCISES_PROGRESS.md)

## 2026-01-01
📖 PCC - `if` Statements (Capitolo 5)  
📖 PCC - Dictionaries (Capitolo 6)  
🎥 PPB - Conditionals & Booleans  
🎥 PPB - Loops & Iterations  
🧑🏼‍🏫 CS50P - Conditionals [[Lezione 1]](../04-COURSES/CS50P/Lecture01.md)   
🔍 [Keywords](../03-CONCEPTS/Keywords.md)  
🔍 [Shadowing](../03-CONCEPTS/Shadowing.md)   

## 2026-01-02
📐 `IterativeLogAnalysis.py` [[Code Review]](../06-PROJECTS/02-SignalProcessor/02_report_Iterative_log_analysis.md)  

## 2026-01-03
🔍 [List Comprehensions](../03-CONCEPTS/ListComprehension.md)    
📝 `if` Statements (5 script)  
📝 List Comprehenesions (8 script)  
📦 Riorganizzata struttura con cartelle specifiche

## 2026-01-05
🔍 [Flusso di Controllo](../03-CONCEPTS/FlussoControllo_Condizionali.md)  
📝 Dizionari (2 script)  

## 2026-01-06
📝 Dizionari (6 script)  
🔍 [Guard Clause Patterns](../03-CONCEPTS/GuardClausePattern.md)  
💭 Conditionals

## 2026-01-08
📖 PCC - User `input()` and `while` Loops (Capitolo 7)  

## 2026-01-09
📖 TP - Introduzione    
📖 TP - La programmazione come forma mentale (Capitolo 1)  
📖 TP - Variabili e Istruzioni (Capitolo 2)

## 2026-01-10
📝 Cicli `while` (2 script)  
🔍 Pattern "Loop and a Half" ✏️da scrivere  

## 2026-01-11
📝 Cicli `while` (4 script)  
🔍 [Slicing](../03-CONCEPTS/Slicing.md)  
📝 Slicing (7 script)  
📦 Riorganizzata struttura [📂EXERCISES](../02-EXERCISES)  
📦 Aggiunto [PIANO_APPRENDIMENTO.md](PIANO_APPRENDIMENTO.md)  

## 2026-01-13
📖 TP - Funzioni  
🔰 Cisco PE1 - Intro, Environment (Modulo 1)  
🔰 Cisco PE1 - Data Types (Modulo 2)  

## 2026-01-14
📐 `AggregatedTelemetryAnalyzer.py` [[Code Review]](../06-PROJECTS/03-AggregatedLogParser/report_claude.md)  
📦 Aggiunto [LOG.md](LOG.md)